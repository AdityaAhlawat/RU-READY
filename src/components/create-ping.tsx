...
export default function CreatePing() {
  ...
  const [formData, setFormData] = useState({
    ...
    time: '',
    date: '',
    durationHours: '', // Separate fields for hours and minutes
    durationMinutes: '',
    happeningNow: false,
  });
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    // Update map center when campus location changes
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/pings', {
        ...
        body: JSON.stringify({ ...formData, duration, user: session?.user }), // Include user information
      });
